...
export default function PageTwo({ domain, allDomains = [], onNavigateToDomain }: InfoCardProps) {
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex border-b border-white/10 space-x-6">
            {[
              ...
              const Icon = tab.icon;
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`pb-3 text-sm font-mono tracking-wider flex items-center gap-2 border-b-2 transition-all cursor-pointer ${activeTab === tab.id ? "border-gold-vintage text-gold-vintage" : "border-transparent text-slate-400 hover:text-white"}`}>
              ...
              {activeTab === "teachings" && (
                <motion.div key="teachings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                  <div className="prose prose-invert max-w-none space-y-4">
                    <p className="text-slate-300 leading-relaxed font-sans text-base">{domain.description}</p>
                  </div>
                ...
                <motion.div key="practice" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                  <div className="p-6 rounded-2xl glass-panel-gold flex flex-col items-center justify-center p-8 text-center border-gold-vintage/30 relative overflow-hidden">
                    ...
                    <div className="flex gap-4">
                      {!breathingActive ? (
                        <button onClick={() => setBreathingActive(true)} className="px-6 py-2 rounded-full bg-gold-vintage hover:bg-gold-bright text-black font-mono text-xs tracking-wider font-semibold transition-all flex items-center gap-2 cursor-pointer"><Play className="w-3.5 h-3.5 fill-black" /><span>BEGIN PATH</span></button>
                      ) : (
                        <button onClick={() => setBreathingActive(false)} className="px-6 py-2 rounded-full bg-white/10 hover:bg-white/20 text-white font-mono text-xs tracking-wider font-semibold transition-all flex items-center gap-2 cursor-pointer"><Square className="w-3.5 h-3.5 fill-white" /><span>HALT PRACTICE</span></button>
                      )}
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      {domain.practiceSteps?.map((step, idx) => (
                        <div key={idx} className="p-4 rounded-lg bg-white/[0.02] border border-white/5 flex items-start gap-3">
                          <span className="font-mono text-xs text-gold-vintage/60 shrink-0 mt-0.5">0{idx + 1}.</span>
        ...
        <div className="space-y-6">
          <div className="p-5 rounded-2xl glass-panel border-white/10 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {images.map((img, idx) => (
                <div key={idx} className="rounded-lg overflow-hidden h-24 relative group cursor-pointer border border-white/5">
                  <img src={img} alt="Cosmic illustration" className="w-full h-full object-cover filter brightness-75 group-hover:scale-110 transition-transform duration-500" />
                  <div className="absolute inset-0 bg-gold-vintage/10 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="p-5 rounded-2xl glass-panel border-white/10 space-y-4">
              ...
              <div className="space-y-3">
                {relatedDomains.map((r) => (
                  <button key={r.slug} onClick={() => onNavigateToDomain?.(r)} className="w-full p-4 rounded-xl border border-white/5 bg-white/[0.02] hover:bg-gold-vintage/[0.04] hover:border-gold-vintage/30 flex items-center justify-between text-left transition-all group cursor-pointer">
                    <div>
                      <h5 className="font-display font-medium text-sm text-slate-200 group-hover:text-gold-vintage transition-colors">{r.title}</h5>
                      <p className="text-[10px] text-slate-400 font-serif italic mt-0.5">{r.subtitle}</p>
                    </div>
