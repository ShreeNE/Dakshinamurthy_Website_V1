...
export default function PageFive({ domain, allDomains = [], onNavigateToDomain }: InfoCardProps) {
  ...
  useEffect(() => {
    ...
    if (breathingActive) {
      timerId = setInterval(() => {
        setBreathTimer((prev) => {
          if (prev === 1) { setBreathPhase((cp) => { switch (cp) { case "Inhale": return "Hold (Full)"; case "Hold (Full)": return "Exhale"; case "Exhale": return "Hold (Empty)"; case "Hold (Empty)": return "Inhale"; default: return "Inhale"; } }); return 4; }
          return prev - 1;
        });
      }, 1000);
